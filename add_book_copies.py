...
# def main():
  ...
  def addBtnClick():
    if isbn.get()!='' and book_title.get()!='' and author.get()!='' and publisher.get()!='' and edition.get()!='' and avail_copies.get()!='':
      if copies.get()=='':
        ...
        try:
          backend.add_book_copies(bookID.get(), copies.get())
          messagebox.showinfo("Done", "Book copies added successfully!", parent=win)
          win.destroy()
  ...
  win.resizable(0, 0)
  bgimage=PhotoImage(file="images/add_book_copies.png") 
  ...
  copies.place(x=380,y=412)


  addBtn = Button(win, text="Add Copies", bg='#655bd6', fg='white', font=(fontUsed, 14), borderwidth=0, width=12, command=addBtnClick)
  addBtn.place(x=545,y=445)
